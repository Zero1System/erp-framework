...
@RequestMapping(value = "/erpsfamilymember")
public class ErpSFamilyMemberController {

    Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
    private ErpSFamilyMemberService service;


    @RequestMapping(value = "/get", method = RequestMethod.GET)
    @ResponseBody
    public ResultBean<ErpSFamilyMember> get(String id) {
        ErpSFamilyMember item = (ErpSFamilyMember) service.selectByPrimaryKey(id);
        if (item != null) {
    ...
    @RequestMapping(value = "/getByStuId", method = RequestMethod.GET)
    @ResponseBody
    public ResultBean<List<ErpSFamilyMember>> getByStuId(String stuId) {
        List<ErpSFamilyMember> item = service.findFamilyMemberByStuId(stuId);
        if (item != null) {
            return new ResultBean<>(item);
        } else {
            return new ResultBean<>(ExceptionEnum.RESOURCE_NOT_FOUND, null, "找不到该记录", null);
    ...
    @ResponseBody
    public ResultBean<String> create(@RequestBody @Validated ErpSFamilyMember item) {
    ...
    @ResponseBody
    public ResultBean<String> update(@RequestBody @Validated ErpSFamilyMember item) {
    ...
    @ResponseBody
    public ResultBean<Integer> delete(@RequestBody ErpSFamilyMember item) {
